...
def remain():
	...
	vvvk = getTileMags('ADP.2014-11-12T16:17:25.027.fits')

	vvvj = getTileMags('ADP.2014-11-12T16:17:57.550.fits')
	
	hdu2mass = fits.open('2mass_b278.fits')
	
	mass2 = hdu2mass[1].data
	
	indices = Parallel(n_jobs=-3, verbose=5)(delayed(minFind)(index,vvvj,vsource) for index,vsource in enumerate(vvvk))
	...
	bt.writeto('vvv_b278.fit',overwrite=True)
...
def main():
	...
	hduvvv = fits.open('vvv_b278.fit')
	
	vvv = hduvvv[1].data
	
	#plt.plot(vvv['JMAG'] - vvv['KMAG'],vvv['KMAG'],'.')
	
	indices = Parallel(n_jobs=-3, verbose=5)(delayed(minFind)(index,mass2,vsource) for index,vsource in enumerate(vvv))
	...
	bt.writeto('vvv_2mass_b278.fit',overwrite=True)
	
def calibrate2mass():
	
	plt.close('all')
	
	hdudata = fits.open('vvv_2mass_b278.fit')
	...
	data = data[data['KMAG_VVV'] > 12.0]
	
	data = data[data['KMAG_VVV'] < 13.0]	
	...
	hist,xedges,yedges = np.histogram2d(sources['JMAG'] - sources['KMAG'], sources['KMAG'] ,[1000,1000],normed=True)
