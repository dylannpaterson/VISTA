...
def remain():
	...
	vvvk = getTileMags('ADP.2014-11-12T16:17:49.603.fits')

	vvvj = getTileMags('ADP.2014-11-12T16:18:47.587.fits')
	
	hdu2mass = fits.open('2mass_gc.fits')
	
	mass2 = hdu2mass[1].data

	mass2 = np.copy(mass2[mass2['GLAT'] < np.max(vvvk['GLAT'])])
	
	mass2 = np.copy(mass2[mass2['GLAT'] > np.min(vvvk['GLAT'])])
	
	mass2 = np.copy(mass2[mass2['GLON'] < np.max(vvvk['GLON'])])
	
	mass2 = np.copy(mass2[mass2['GLON'] > np.min(vvvk['GLON'])])
	
	indices = Parallel(n_jobs=-3, verbose=5,backend='threading')(delayed(minFind)(index,vvvj,vsource) for index,vsource in enumerate(vvvk))
	...
	bt.writeto('vvv_b236.fit',overwrite=True)
...
def main():
	...
	indices = Parallel(n_jobs=-3, verbose=5, backend = 'threading')(delayed(minFind)(index,mass2,vsource) for index,vsource in enumerate(vvv))
	...
	bt.writeto('vvv_2mass_b236.fit',overwrite=True)
	
def calibrate2mass():
	
	plt.close('all')
	
	hdudata = fits.open('vvv_2mass_b236.fit')
	...
	window = sources[np.abs(sources['GLON'] - np.mean(sources['GLON'])) <= 5.0/60.0]

	window = window[np.abs(window['GLAT'] - np.mean(window['GLAT'])) <= 5.0/60.0]
	...
	bt.writeto('b236.fit',overwrite=True)
